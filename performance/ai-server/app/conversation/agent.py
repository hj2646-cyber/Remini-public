...
def _finalize_fire_and_forget(
    ...
    try:
        ...
        # caregiver webhook — WebSocket path (위험발화 → 보호자 폰 PWA 푸시)
        if req.user_id:
            if ctx.crisis_flag:
...
async def _prepare_context(
    ...
    # [perf debug] KG retrieval 결과 가시화
    logger.info(
        "agent.retrieval turn=%d user=%s texts=%d used=%s preview=%r",
        req.turn_id, req.user_id, len(rr.texts), rr.used,
        (rr.texts[0][:80] if rr.texts else None),
    )
    ...
    # 시연 토글: PERF_STT_ROBUST=false 시 이 블록 skip → 환자가 버벅거리면 못 알아먹음
    if _perf.stt_robust():
        try:
            fragment_hint = bundle.retrieval.reconstruct_from_fragments(
                req.text, user_id=req.user_id
            )
            # [perf debug] fragment 재구성 결과
            logger.info(
                "agent.fragment_hint turn=%d hint=%r",
                req.turn_id, (fragment_hint[:80] if fragment_hint else None),
            )
            if fragment_hint:
        ...
        logger.info("agent.fragment_hint turn=%d skipped (PERF_STT_ROBUST=false)", req.turn_id)
...
async def run_agent(
    ...
    # 시연 토글: PERF_REMINISCENCE_PHOTO=false 시 자동 트리거 skip
    triggered_reminiscence: dict | None = None
    if _perf.reminiscence_photo_enabled():
        try:
