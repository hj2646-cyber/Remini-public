...
class TherapyStateTracker:
    ...
    def update(
        ...
        # EXPLORATION (회상 신호가 있을 때 전이 — 일상 대화만이면 OPENING 유지)
        # turn_count 만으로 자동 전이하던 3턴 → 5턴으로 늦춰, 환자가 옛 기억을
        # 꺼내지 않은 평범한 일상 대화에서는 회상 탐색 지침을 주입하지 않는다.
        if st.phase == TherapyPhase.OPENING and (
            st.turn_count >= 5 or st.reminisce_turns >= 1
        ):
...
_PHASE_GUIDANCE: dict[TherapyPhase, str] = {
    ...
    TherapyPhase.EXPLORATION: (
        "[회상요법 단계: EXPLORATION]\n"
        "환자가 옛 기억을 꺼내면 장소 이름/색/냄새/촉감 같은 구체적인 감각 단서를 살짝 덧붙여 "
        "장면을 더 생생하게 키워 주세요. 다만 환자가 지금 일상 이야기에 머물러 있다면 "
        "억지로 과거로 돌리지 말고 그 이야기를 함께 이어가 주세요."
    ),
