...
from app.services.stt_correction import correct_transcript
...
class PatientSession:
    ...
    async def _process_vad_block(self, block_bytes: bytes) -> None:
        # 시연 모드: WS_BARGEIN_DISABLED=true 면 응답 중 (RESPONDING) 마이크 입력 완전 무시.
        # VAD/STT/segment 누적 모두 skip → 환자가 말해도 시스템이 듣지 않음.
        # 응답 끝나고 LISTENING 복귀하면 다시 정상 동작.
        if settings.ws_bargein_disabled and self.state.name == StateName.RESPONDING:
            return
        ...
        if self._in_speech:
            ...
            await self._feed_stt_stream(block_bytes)

            # Barge-in gate: if we're RESPONDING and the user has been
            # vocalizing for a sustained window, fire StartOfTurn.
            # 시연 모드: WS_BARGEIN_DISABLED=true 면 응답 중 환자 발화 완전 무시.
            # (환자가 다시 "말하기 완료" 버튼을 눌러야만 새 turn 시작)
            if (
                not settings.ws_bargein_disabled
                and self.state.name == StateName.RESPONDING
        ...
        if "end" in result:
            ...
            if len(segment) >= (self.cfg.min_speech_ms // 32) * _VAD_BLOCK_BYTES:
                if self.stt.supports_streaming:
                    await self._finish_streaming_segment(segment)
                else:
    ...
    async def _flush_end_of_turn(self) -> None:
        ...
        if self._in_speech and self._speech_buf and self.stt.supports_streaming:
            segment = bytes(self._speech_buf)
            self._speech_buf = bytearray()
            self._in_speech = False
            self._speech_frames_since_start = 0
            await self._finish_streaming_segment(segment)
        elif self._in_speech and self._speech_buf:
            segment = bytes(self._speech_buf)
            self._speech_buf = bytearray()
            self._in_speech = False
            self._speech_frames_since_start = 0
        ...
        if combined:
            # 최종 보정 — LLM 에 들어가는 텍스트를 소형 LLM 이 1회 교정 (STT 오인식 해소).
            if settings.stt_correction_enabled:
                loop = asyncio.get_running_loop()
                try:
                    corrected = await loop.run_in_executor(
                        None, correct_transcript, combined, None,
                    )
                    if corrected:
                        combined = corrected
                except Exception:
                    logger.exception("STT correction failed; using raw transcript")
            self._pending_text = ""
            self._last_interim_text = ""
    ...
    async def _ensure_stt_stream(self) -> bool:
        if not self.stt.supports_streaming:
            return False
        if self._stt_stream is not None:
            return True
        loop = asyncio.get_running_loop()
        try:
            self._stt_stream = await loop.run_in_executor(None, self.stt.begin_stream)
            self._stt_stream_text = ""
            return True
        except Exception:
            logger.exception("Qwen3-ASR streaming session start failed")
            self._stt_stream = None
            return False

    async def _feed_stt_stream(self, block_bytes: bytes) -> None:
        if not self.stt.supports_streaming:
            return
        if not await self._ensure_stt_stream():
            return
        loop = asyncio.get_running_loop()
        try:
            text = await loop.run_in_executor(
                None, self.stt.stream_chunk, self._stt_stream, block_bytes,
            )
        except Exception:
            logger.exception("Qwen3-ASR streaming chunk failed")
            await self._abort_stt_stream()
            return
        text = (text or "").strip()
        if not text:
            return
        self._stt_stream_text = text
        combined = self._join_text(self._pending_text, self._stt_stream_text)
        if combined and combined != self._last_interim_text:
            self._last_interim_text = combined
            await self.event_q.put(Event(EventType.INTERIM_TEXT, {"text": combined}))

    async def _finish_streaming_segment(self, segment_pcm: bytes) -> None:
        loop = asyncio.get_running_loop()

        if self._speaker_verifier.is_enrolled and segment_pcm:
            is_patient, sim = await loop.run_in_executor(
                None, self._speaker_verifier.verify, segment_pcm, _VAD_SAMPLE_RATE,
            )
            if not is_patient:
                logger.info("Speaker rejected (similarity=%.3f), dropping streamed STT", sim)
                await self._abort_stt_stream()
                self._stt_stream_text = ""
                combined = self._pending_text.strip()
                if combined != self._last_interim_text:
                    self._last_interim_text = combined
                    await self.event_q.put(Event(EventType.INTERIM_TEXT, {"text": combined}))
                return
        ...
        if self._stt_stream is None:
            return
        handle = self._stt_stream
        self._stt_stream = None
        try:
            text = await loop.run_in_executor(None, self.stt.finish_stream, handle)
        except Exception:
            logger.exception("Qwen3-ASR streaming finish failed")
            self._stt_stream_text = ""
            return

        text = (text or self._stt_stream_text or "").strip()
        self._stt_stream_text = ""
        if not text:
            return
        self._pending_text = self._join_text(self._pending_text, text)
        if self._pending_text != self._last_interim_text:
            self._last_interim_text = self._pending_text
            await self.event_q.put(Event(EventType.INTERIM_TEXT, {"text": self._pending_text}))

    async def _abort_stt_stream(self) -> None:
        handle = self._stt_stream
        self._stt_stream = None
        self._stt_stream_text = ""
        if handle is None:
            return
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, self.stt.abort_stream, handle)
